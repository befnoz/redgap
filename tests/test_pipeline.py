"""The REPLAY pipeline over the committed real fixtures: the default run detects 34/51
with the flagship on a shipped rule; --fix loads the closing rules and every rule-gap
flips green (46/51), leaving only the 5 base-rate gaps; the result is deterministic -
all offline, no Docker, no API key.
"""
...
def test_replay_default_detected_count(tmp_path):
    ...
    assert report["summary"]["detected"] == 34
    assert report["summary"]["techniques"] == 51
...
def test_fix_closes_every_rule_gap(tmp_path):
    verdicts, report = run_coverage(ReplayTarget(), generated_at=WHEN, out_dir=tmp_path, fix=True)
    by = {v.technique_id: v for v in verdicts}
    assert by["T1070.006"].detected
    assert report["summary"]["detected"] == 46
    # After the closing rules load, ONLY base-rate gaps remain (they need correlation,
    # not a single-event rule) - every rule-gap is closed.
    assert report["summary"]["gaps_by_type"] == {"base_rate": 5}
    for v in verdicts:
        if not v.detected:
            assert v.gap_type is GapType.BASE_RATE, v.technique_id
