...
- **T1201 Password Policy Discovery** - rule: telemetry present, but no rule fired (closeable by writing a rule)
- **T1614.001 System Location Discovery: System Language Discovery** - rule: telemetry present, but no rule fired (closeable by writing a rule)
- **T1613 Container and Resource Discovery** - rule: telemetry present, but no rule fired (closeable by writing a rule)
- **T1070.003 Indicator Removal: Clear Command History** - rule: telemetry present, but no rule fired (closeable by writing a rule)
- **T1543.002 Create or Modify System Process: Systemd Service** - rule: telemetry present, but no rule fired (closeable by writing a rule)
